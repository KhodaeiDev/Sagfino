...
const SelectBox: React.FC<SelectBoxProps> = ({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div className="absolute z-50 top-full min-h-auto max-h-50 overflow-y-auto left-0 right-0 mt-2 p-3 bg-white shadow-lg border border-blue-400 rounded-lg">
          {options.map((option) => (
            <li
              key={option.id}
              onClick={
              ...
              className={`list-none rounded-sm transition-all duration-500 cursor-pointer px-4 py-2 
                ...
                ${option.id === 0 ? 'text-gray-400 pointer-events-none' : ''}`} // ✅ غیرفعال کردن انتخاب گزینه‌ی راهنما.
            >
              {option.name}
            </li>
          ))}
